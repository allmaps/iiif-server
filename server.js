...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (arg === '--containers-dir') {
      ...
      console.log('Usage: node server.js [--containers-dir containers] [--host 127.0.0.1] [--port 8000] [--tls-key certs/dev.key --tls-cert certs/dev.crt]');
      process.exit(0);
    }
  }
...
function buildOperationPlan(record, region, size) {
  ...
  const left = Math.max(0, Math.min(pageWidth - 1, Math.floor(region.x / selected.scale)));
  const top = Math.max(0, Math.min(pageHeight - 1, Math.floor(region.y / selected.scale)));
  const right = Math.max(left + 1, Math.min(pageWidth, Math.ceil((region.x + region.w) / selected.scale)));
  const bottom = Math.max(top + 1, Math.min(pageHeight, Math.ceil((region.y + region.h) / selected.scale)));
...
async function renderImage(record, region, size, rotation, quality, fmt, sharpFactory = getSharp()) {
  const render = async (usePyramidPage) => {
    ...
    const isFullRegion = region.x === 0 && region.y === 0 && region.w === record.width && region.h === record.height;
    if (usePyramidPage) {
    ...
    const isFull = extractRegion.left === undefined && extractRegion.x === 0 && extractRegion.y === 0 &&
      extractRegion.w === record.width && extractRegion.h === record.height;
...
async function loadCatalog(containersDir, sharpFactory = getSharp()) {
  ...
  for (const entry of entries) {
    ...
    const metadata = await sharpFactory(fullPath, { limitInputPixels: false }).metadata();
  ...
  return {
    ...
    profile: [
      'http://iiif.io/api/image/2/level1.json',
      {
        ...
        supports: ['regionByPx', 'sizeByW', 'sizeByH', 'sizeByPct', 'sizeByWh', 'rotationBy90s'],
      },
    ],
  ...
  return async (req, res) => {
    try {
      ...
      if (parts.length === 2 && parts[1] === 'info.json') {
        ...
        sendJson(res, 200, infoJson(`${scheme}://${host}`, imageId, record.width, record.height));
        return;
      }
      ...
      const rendered = await renderImage(record, region, size, rotation, quality, fmt, sharpFactory);
