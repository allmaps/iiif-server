...
This repository contains a script to generate pyramidal TIFFs using [Sharp](https://sharp.pixelplumbing.com/). Then it has two servers that create a IIIF Image API that efficiently reads from this source file. The most interesting is the "Level 0 Byte-Streaming Server" which offers a `level0` compliance API. It extracts the relevant tiles from the TIF without reading the entire file. Only for the edge tiles (with a deviating width and/or height) it decodes and encodes the image.
